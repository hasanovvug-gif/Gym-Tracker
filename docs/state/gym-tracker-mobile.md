...
**Открытый вопрос — иконка приложения.** Показал 4 концепта, Вугар выбор отложил
до новой сессии. Исходники и таблица «суть / риск» — `design/icon-concepts/`,
рекомендация Клода там же (вариант B, монограмма на Oswald).

Дальше по приоритету:
1. Дождаться выбора иконки → собрать 1024×1024, adaptive под Android, splash
   под обе темы. Пошаговый чеклист — `design/icon-concepts/README.md`.
...
- [ ] **Выбрать иконку из 4 концептов** (`design/icon-concepts/`), затем собрать
      иконку + adaptive + splash — сейчас дефолтные ассеты Expo
...
- Last meaningful commit: `f14195b` — звуковой сигнал, haptics, анимации (запушен)
