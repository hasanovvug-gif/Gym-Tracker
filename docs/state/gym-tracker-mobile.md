...
Всё закоммичено и запушено в `main` (`f14195b`), рабочее дерево чистое.
Полный флоу Вугар прогнал на реальном айфоне — работает.

Дальше по приоритету:
1. Дать бипу и вибрации оценку вживую на телефоне: тембр/громкость сигнала и
   не слишком ли частая вибрация (дефолт нажатия — `light`, крутилки веса тоже).
