...
## Next step (завтра, продолжаем feat/live-activity)

1. [ ] **Time-aware кнопка после отдыха** (native, вариант 1): в `targets/live-activity/GymbarLiveActivity.swift`
   — `CompleteSetButton` показывать когда `canCompleteSet || (phase=="rest" && restEndsAt<=now)`; ставить
   `staleDate = restEndsAt` при старте отдыха (в `_shared/CompleteSetIntent.swift` и в модуле `updateLiveActivity`);
   ослабить guard в `_shared/CompleteSetIntent.swift` `perform()` чтобы принимать тап при истёкшем отдыхе.
2. [ ] Убрать `os_log`/`intentLog` из `_shared/CompleteSetIntent.swift` (диагностика, больше не нужна).
3. [ ] Пересборка `npx expo run:ios --device` (в терминале Вугара — UTF-8 локаль) + тест на устройстве.
4. [ ] Когда стабильно — влить `feat/live-activity` в `main`, собрать build #3 (EAS), отправить в App Store.
